-- Enforce per-game score caps for display/fallback score submission.
...
begin
  ...
  v_max_score := case p_game_key
    when 'interval1' then 10
    when 'interval2' then 10
    when 'harmony1' then 10
    when 'harmony2' then 10
    when 'harmony3' then 10
    when 'tempo1' then 100
    when 'tempo2' then 100
    when 'pitch1' then 100
    when 'rhythm1' then 100
    else 1000
  end;

  if p_score is null or p_score < 0 or p_score > v_max_score then
    raise exception 'Invalid score value for game key % (max %)', p_game_key, v_max_score;
  end if;

  v_final := least(v_max_score, p_score)::numeric;
